...
public class WordMemorize {
    ...
    void Run() throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        while (!finished) {
            ...
            if (commands.containsKey(data[0])) {
                ...
                try {
                    this.getClass().getDeclaredMethod(data[0].substring(1),
                            commands.get(data[0])).invoke(this, args.toArray());
    ...
    void generate(int number) {
        ...
        for (int i = 0; i < number; i++) {
            wordSize = r.nextInt(10) + 1;
            nextWord.setLength(0);
            for (int j = 0; j < wordSize; j++) {
                nextWord.append((char) r.nextInt(256));
            }
            list.add(nextWord.toString());
        }
